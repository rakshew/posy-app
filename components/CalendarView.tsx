    ...
    <div className="py-4 md:py-8 float-up space-y-20 md:space-y-32 px-4 md:px-0">
      <section>
        <div className="flex justify-between items-center mb-10 md:mb-20">
          <button onClick={prevMonth} className="p-4 text-posy-ink/20 hover:text-posy-purple transition-all">
          ...
          <div className="text-center">
             ...
             <p className="text-[10px] font-medium text-posy-ink/20 uppercase tracking-[0.6em]">{year}</p>
          </div>
          <button onClick={nextMonth} className="p-4 text-posy-ink/20 hover:text-posy-purple transition-all">
        ...
        <div className="grid grid-cols-7 gap-3 md:gap-6 mb-8">
          {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
            <div key={`${d}-${i}`} className="text-center text-[10px] font-medium text-posy-ink/20 uppercase tracking-[0.4em]">{d}</div>
          ))}
        </div>
              ...
              <div key={day} className="relative aspect-square">
                ...
                >
                  {entry ? (
                    ...
                    <span className="text-lg md:text-2xl font-serif text-posy-ink/20 font-light group-hover:text-posy-purple transition-colors">
                      {day}
                    </span>
                  )}
                </button>
        ...
        <div className="posy-card p-10 md:p-16 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-8">
              <p className="text-[9px] font-medium uppercase tracking-widest text-posy-ink/20">Flora Snapshot</p>
              <div className="space-y-5">
                {Object.entries(monthMoodCounts).length === 0 ? (
                  <p className="italic text-posy-ink/20 font-serif text-2xl font-light">A quiet garden waits...</p>
                ) : (
